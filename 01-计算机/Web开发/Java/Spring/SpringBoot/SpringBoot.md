...
* 选择 `Spring Initializr` ，用来创建 `SpringBoot` 工程，在 `Module SDK` 这一项选择我们安装的 `JDK` 版本。
...
为了更改项目配置，`SpringBoot`提供了多种属性配置方式
...
1. 创建模块，创建新模块，选择 `Spring Initializr`，并配置模块相关基础信息
...
2. 定义实体类，在 `com.itheima.domain` 包下定义实体类 `Book`，内容如下
...
3. 定义dao接口，在 `com.itheima.dao` 包下定义 `BookDao` 接口，内容如下
...
4. 定义测试类，在 `test/java` 下定义包 `com.itheima` ，在该包下测试类，内容如下
